...
(function () {
  ...
  // No IDs found — every caller returns this same empty shape on a failure/early-out path.
  const NO_PLAG_IDS = { strong: /** @type {string[]} */ ([]), weak: /** @type {string[]} */ ([]) };
  ...
  //
  // Returns { text, plagIds }. plagIds are Tableau's own internal autogenerated identifiers,
  // not anything a student writes or sees in the UI, mined for the plagiarism checker:
  //   strong — <column name="[Calculation_XXXXXXXXXXXXXXXX]"> internal calc-field IDs.
  //     Tableau mints a fresh one of these the moment a calculated field is created; there is
  //     no mechanism by which two independently-authored calcs land on the same number. A
  //     match across two submissions means one workbook's calc was literally copied from the
  //     other (Save As / duplicate / paste), not that two students wrote similar formulas.
  //   weak — <datasource name="..."> internal connection IDs. Also autogenerated, but if the
  //     assignment handed out a shared starter .twbx/.tds that already had the datasource
  //     connected, every honest submission inherits the SAME datasource ID through no fault
  //     of the student — so a match here is only supporting evidence, never proof on its own.
  function twbxToText(/** @type {ArrayBuffer} */ buffer) {
    try {
      ...
      if (!twbName) {
        ...
        return { text: `[EXTRACTION FAILED: Tableau workbook — no .twb file found inside .twbx. This is a tooling limitation, not evidence the submission is empty — do not grade this as zero content.]`, plagIds: NO_PLAG_IDS };
      }
      ...
      if (doc.querySelector('parsererror')) {
        ...
        return { text: `[EXTRACTION FAILED: Tableau workbook — .twb XML could not be parsed. This is a tooling limitation, not evidence the submission is empty — do not grade this as zero content.]`, plagIds: NO_PLAG_IDS };
      }
      ...
      const calcIdRe = /^\[Calculation_\d+\]$/;
      const strongIds = /** @type {string[]} */ ([]);
      const calcs = [...doc.querySelectorAll('column')]
        .map(col => {
          ...
          if (calcIdRe.test(internalName)) strongIds.push(internalName);
          const label = col.getAttribute('caption') || internalName || '(unnamed)';
          return `"${label}" = ${calc.getAttribute('formula')}`;
        })
        .filter(Boolean)
        .slice(0, 40);

      const weakIds = [...doc.querySelectorAll('datasource[name]')]
        .map(ds => ds.getAttribute('name'))
        .filter(/** @type {(v: string|null) => v is string} */ (Boolean));
      ...
      if (parts.length === 1) {
        ...
        return { text: `[EXTRACTION FAILED: Tableau workbook — parsed the .twb file but found no worksheets, dashboards, calculated fields, or filters. This is almost certainly a tooling limitation, not evidence the submission is empty — do not grade this as zero content.]`, plagIds: NO_PLAG_IDS };
      }
      return { text: parts.join('\n\n'), plagIds: { strong: strongIds, weak: weakIds } };
    } catch (e) {
      console.warn('[MAG] Tableau: unexpected error extracting .twbx:', e);
      return { text: `[EXTRACTION FAILED: Tableau workbook — could not parse (${/** @type {any} */(e).message}). This is a tooling limitation, not evidence the submission is empty — do not grade this as zero content.]`, plagIds: NO_PLAG_IDS };
    }
  }
  ...
  //
  // The page/visual IDs in the folder names themselves (pageId, visualId) are Power BI
  // Desktop's own autogenerated internal object IDs, minted fresh the moment a page or
  // visual is created — not something a student types or even sees. The DAX/data-model
  // layer (where a Tableau-style Calculation_ ID would live) is in a proprietary binary
  // format this script can't reach (see pbixToText's comment), but these folder IDs are
  // plain text we already enumerate, and they're just as high-entropy: two independently
  // built reports landing on the same ID by chance is not realistic. Treated as "strong"
  // plagiarism evidence, same tier as Tableau's Calculation_ IDs.
  function pbirPagesToText(/** @type {Record<string,Uint8Array>} */ inner, /** @type {string[]} */ entryNames) {
    ...
    const pages = orderedEntries.map(pageEntryName => {
      ...
      const visuals = visualEntries.map(visEntryName => {
        const visualId = (visEntryName.match(visualJsonRe) || [])[2];
        if (visualId) strongIds.push(`visual:${visualId}`);
    ...
    return { text: `[POWER BI REPORT]\n${POWER_BI_NOTE}\n\n${pages.join('\n\n')}`, plagIds: { strong: strongIds, weak: [] } };
  }
  ...
  function pbixLegacyLayoutToText(/** @type {Record<string,Uint8Array>} */ inner, /** @type {string[]} */ entryNames) {
    ...
    // section.name / visualContainer.name are Power BI Desktop's own internal object IDs
    // (GUIDs), same role as the PBIR folder IDs above — autogenerated per page/visual,
    // not student-authored. Same "strong" plagiarism-evidence tier.
    const strongIds = /** @type {string[]} */ ([]);
    const pages = (layout.sections || []).map((/** @type {any} */ section) => {
      if (section.name) strongIds.push(`page:${section.name}`);
      const pageName = section.displayName || section.name || '(unnamed page)';
      const visuals = (section.visualContainers || []).map((/** @type {any} */ vc) => {
        if (vc.name) strongIds.push(`visual:${vc.name}`);
  ...
  function pbixToText(/** @type {ArrayBuffer} */ buffer) {
    try {
      ...
      return { text: `[EXTRACTION FAILED: Power BI report — recognized neither the PBIR nor the legacy internal report structure. This is a tooling limitation, not evidence the submission is empty — do not grade this as zero content.]`, plagIds: NO_PLAG_IDS };
    } catch (e) {
      console.warn('[MAG] Power BI: unexpected error extracting .pbix:', e);
      return { text: `[EXTRACTION FAILED: Power BI report — could not parse (${/** @type {any} */(e).message}). This is a tooling limitation, not evidence the submission is empty — do not grade this as zero content.]`, plagIds: NO_PLAG_IDS };
    }
  }
  ...
  async function extractZip(/** @type {ArrayBuffer} */ buffer) {
    ...
    const parts      = [];
    const errorNotes = [];
    const plagIdsAcc = { strong: /** @type {string[]} */ ([]), weak: /** @type {string[]} */ ([]) };
    ...
    for (const [name, data] of entries) {
      ...
      try {
        if (ext === 'csv') {
          ...
          const r = twbxToText(ab);
          text = r.text;
          plagIdsAcc.strong.push(...r.plagIds.strong);
          plagIdsAcc.weak.push(...r.plagIds.weak);
        } else if (ext === 'pbix') {
          const r = pbixToText(ab);
          text = r.text;
          plagIdsAcc.strong.push(...r.plagIds.strong);
          plagIdsAcc.weak.push(...r.plagIds.weak);
        } else {
    ...
    if (!parts.length) return { text: null, innerFilenames, plagIds: plagIdsAcc };

    let text = parts.join('');
    if (errorNotes.length) text += `\n\n[Note: ${errorNotes.join('; ')}]`;
    return { text, innerFilenames, plagIds: plagIdsAcc };
  }
  ...
  async function extractSubmission(fileUrl, filename) {
    ...
    if (ext === 'twbx') {
      const r = twbxToText(buffer);
      return { text: r.text, inlineData: null, plagIds: r.plagIds };
    }
    if (ext === 'pbix') {
      const r = pbixToText(buffer);
      return { text: r.text, inlineData: null, plagIds: r.plagIds };
    }
    if (isZip || mime.includes('zip') || mime.includes('x-zip')) {
      const { text: zipText, innerFilenames, plagIds } = await extractZip(buffer);
      return { text: zipText, inlineData: null, innerFilenames, plagIds };
    }
...
Your response is the JSON object described above, and nothing else. Do not explain your reasoning. Do not narrate what you are deciding or checking. Do not write about the rules, the criteria, or what you should or should not include. No markdown fences, no prose before or after the JSON. If you notice yourself writing anything other than the JSON object (or "feedback": []), stop and delete it. Output ONLY the final JSON.`;
  ...
  function groundFeedback(/** @type {any[]} */ items, /** @type {string} */ submissionText) {
    ...
    for (const item of items || []) {
      ...
      // Same problem at the other end: the model sometimes writes an item lowercase,
      // as if it were a continuation of the previous one, even though each item is
      // joined as its own standalone sentence — producing "...visualizations. the
      // dual-axis chart..." after the period above. Capitalize only the first letter
      // (not capFirst's whole-word lowercase-the-rest, which would mangle "Tableau"/
      // "SQL" mid-sentence); leave non-letter openers (a digit, quote) untouched.
      const capitalized = /^[a-z]/.test(punctuated)
        ? punctuated.charAt(0).toUpperCase() + punctuated.slice(1)
        : punctuated;
      kept.push(capitalized);
    }
...
Respond with ONLY the rewritten sentence. No quotes, no explanation, no markdown.`;
  ...
  // Intersection of two internal-ID arrays (Tableau Calculation_ fields, datasource
  // connections, Power BI page/visual object IDs — see twbxToText/pbixToText).
  function plagIdIntersect(/** @type {string[]} */ a, /** @type {string[]} */ b) {
    if (!a?.length || !b?.length) return [];
    const setB = new Set(b);
    return [...new Set(a)].filter(id => setB.has(id));
  }

  function computePlagiarismPairs(/** @type {Record<string,{name:string,extractedText:string,plagIds?:{strong:string[],weak:string[]}}>} */ cache) {
    const entries = Object.entries(cache).filter(([, v]) => v.extractedText);
    const pairs = /** @type {{sA:any,sB:any,score:number,isBiTool:boolean,idMatch:{strong:string[],weak:string[]}}[]} */ ([]);
    for (let i = 0; i < entries.length; i++) {
      for (let j = i + 1; j < entries.length; j++) {
        ...
        const idMatch = {
          strong: plagIdIntersect(dataA.plagIds?.strong || [], dataB.plagIds?.strong || []),
          weak:   plagIdIntersect(dataA.plagIds?.weak   || [], dataB.plagIds?.weak   || []),
        };
        pairs.push({ sA: { uid: uidA, name: dataA.name }, sB: { uid: uidB, name: dataB.name }, score, isBiTool, idMatch });
      }
    }
    // A shared "strong" internal ID is decisive regardless of text-similarity score — sort
    // those to the top ahead of pure text-similarity ranking.
    return pairs.sort((a, b) => (b.idMatch.strong.length > 0 ? 1 : 0) - (a.idMatch.strong.length > 0 ? 1 : 0) || b.score - a.score);
  }

  function openPlagiarismPanel(/** @type {Record<string,{name:string,extractedText:string,plagIds?:{strong:string[],weak:string[]}}>} */ cache) {
    ...
    // A shared "strong" internal ID (Tableau Calculation_ field, Power BI page/visual
    // object ID) is surfaced regardless of text-similarity score — it's independent,
    // near-conclusive evidence that doesn't depend on how much surrounding text overlaps.
    const visible     = pairs.filter(p => p.score >= showMinFor(p) || p.idMatch.strong.length > 0);
    ...
    // Strong IDs (Calculation_ fields / page & visual object IDs) are Tableau's/Power BI's
    // own autogenerated internals — not something a student writes or two independent
    // authors could land on by chance. Weak IDs (shared datasource connection) are only
    // supporting evidence, since a shared class starter file would produce the same match
    // for every honest submission too.
    const idBadge = (/** @type {{strong:string[],weak:string[]}} */ idMatch) => {
      if (idMatch.strong.length) {
        return `<span class="mag-plag-idmatch mag-plag-idmatch-strong" title="${idMatch.strong.join(', ')}">🚨 Shared internal ID (near-certain — one file was derived from the other, unless both descend from a shared class starter file)</span>`;
      }
      if (idMatch.weak.length) {
        return `<span class="mag-plag-idmatch mag-plag-idmatch-weak" title="${idMatch.weak.join(', ')}">⚠ Shared datasource ID (supporting only — expected if this assignment provided a shared starter file)</span>`;
      }
      return '';
    };
    ...
    const below = pairs.filter(p => p.score < showMinFor(p) && p.idMatch.strong.length === 0);
      ...
      <div class="mag-plag-box">
        ...
        <div class="mag-plag-body">
          <div class="mag-plag-meta">
            ...
            &nbsp;·&nbsp; 5-gram Jaccard similarity + internal-ID matching (Tableau/Power BI) &nbsp;·&nbsp; 0 API calls used
          </div>
          ${rowsHtml}
          ${below.length ? `
    ...
    for (const btn of /** @type {NodeListOf<HTMLButtonElement>} */(overlay.querySelectorAll('.mag-plag-ai-btn'))) {
      btn.addEventListener('click', async () => {
        ...
        try {
          ...
          const idMatchNote = pair.idMatch.strong.length
            ? `\nDECISIVE EVIDENCE: both submissions contain the exact same internal autogenerated identifier(s) that Tableau/Power BI mint fresh per object and a student never sees or types: ${pair.idMatch.strong.join(', ')}. Independent authors cannot land on the same one by chance — this means one file was literally derived from the other (copy, Save As, or duplicate), UNLESS both descend from a shared starter file the instructor distributed for this assignment (you don't know which — say so in "reason" if you can't tell). Weigh this far more heavily than the text-similarity score.`
            : pair.idMatch.weak.length
              ? `\nNote: both submissions share the same internal datasource connection ID (${pair.idMatch.weak.join(', ')}). This is expected and NOT evidence of copying if the assignment provided a shared starter file with the datasource pre-connected — only treat it as relevant if you also find independent corroborating similarity.`
              : '';
          const prompt =
...
${checkInstructions}`;
  ...
  GM_addStyle(`
    ...
    .mag-plag-idmatch {
      font-size: 10px; padding: 2px 7px; border-radius: 10px; cursor: help;
    }
    .mag-plag-idmatch-strong { background: rgba(220,20,20,0.35); color: #ff8080; }
    .mag-plag-idmatch-weak   { background: rgba(180,130,20,0.30); color: #ffc050; }
  ...
  function wireCardButtons(student, rubric, result, assignmentId) {
    ...
    postBtn.onclick = async () => {
      ...
      // Re-reads the panel's CURRENT score/justification/feedback DOM into a fresh result
      // object. Called both at initial post and again from Stay/Move, since the user can
      // keep adjusting the selects/textareas after the first post — Stay/Move must persist
      // whatever's in the panel NOW, not whatever it was at the moment postBtn was clicked.
      const collectEditedResult = () => {
        ...
        return { ...result, scores: editedScores, feedback: editedFeedback };
      };

      try {
        const editedResult = collectEditedResult();

        // Auto-grade path: apply grades to Moodle's live AMD grader state (rubric cell
        // clicks + TinyMCE), then let saveandshownext persist them. This mirrors exactly
        ...
        // and so Stay/Move (which now do their own fresh immediate apply — see below)
        // don't race against this stale-data retry.
        ...
        if (resultRow && doneBtn) {
          ...
          if (stayBtn) stayBtn.onclick = async () => {
            ...
            // Cancel the pending 2-second stale-data retry (scheduled at the initial post,
            // closed over the OLD editedResult) — it would otherwise fire after our fresh
            // apply below and clobber a manual adjustment back to the original values.
            _cancelLiveTimers();
            // Re-collect: the user may have adjusted a score/justification/feedback in the
            // panel after the initial post. Moodle's "Save changes" button below only
            // re-submits whatever is CURRENTLY in its own live DOM, so that DOM must be
            // refreshed from the panel's current values now, or a post-post adjustment is
            // silently dropped (v2.6.31). immediate:true awaits real completion, no guessing.
            const liveResult = collectEditedResult();
          ...
          if (moveBtn) moveBtn.onclick = async () => {
            ...
            // Cancel the pending 2-second full-reapply retry (scheduled when the grade was
            // first posted, above) — we're about to do a fresh apply right now instead.
            _cancelLiveTimers();

            // Re-collect: the user may have adjusted a score/justification/feedback in the
            // panel after the initial post — Moodle's live DOM must reflect that before we
            // navigate away, or the adjustment is silently dropped (v2.6.31). Console
            // evidence (v2.5.72) showed Moodle's OWN GradingPanel code throwing an uncaught
            // exception in its post-save dirty-flag cleanup (getFormFromChild reading
            // .closest on undefined), immediately followed by the navigation storm — a
            // genuine bug in Moodle's grading panel JS, exposed by navigating while writes
            // are still in flight. immediate:true awaits the real write completion instead
            // of guessing a fixed delay, so Moodle's internal state has settled by the time
            // this click reaches it.
            const liveResult = collectEditedResult();
            await applyResultToLiveDom(rubric, liveResult, { immediate: true });
            await sleep(200);
        ...
        // In auto-grade mode, don't halt on a post error — skip via next-user (no save,
        // so a partial or corrupted AMD state doesn't overwrite the student's existing grade).
        autoGradeThisPost = false;
  ...
  async function runGradeOne() {
    ...
    const gradeCurrentStudent = async () => {
      ...
      try {
        ...
        // Tableau/Power BI internal autogenerated IDs (Calculation_ fields, datasource
        // connections, page/visual object IDs) mined for the plagiarism checker — see
        // twbxToText/pbixToText. Accumulated across every file this student submitted.
        const plagIds = { strong: /** @type {string[]} */ ([]), weak: /** @type {string[]} */ ([]) };
        if (fileLinks.length) {
          const parts = [];
          for (let _fi = 0; _fi < fileLinks.length; _fi++) {
            ...
            try {
              ...
              if (extracted.plagIds) {
                plagIds.strong.push(...extracted.plagIds.strong);
                plagIds.weak.push(...extracted.plagIds.weak);
              }
        ...
        plagiarismCache[student.uid] = { name: student.name, extractedText: submissionText, plagIds };
        _savePlagCache();
    ...
    const onMoodleNavigated = async (/** @type {string} */ newUid) => {
      ...
      const autoAdvance = async () => {
        ...
        await sleep(600);
        // Use next-user (no form save) — we are skipping, not grading, so we must not
        // submit the AMD form which would overwrite an already-graded student's rubric.
        const mn = /** @type {HTMLElement|null} */(document.querySelector('[data-action="next-user"], [data-action="nextuser"]'));
    ...
    const navWatcher = setInterval(() => {
      if (!reviewOverlay.classList.contains('open')) { clearInterval(navWatcher); activeGradeCurrentFn = null; gradeAllActive = false; gradeNRemaining = 0; autoGradeThisPost = false; return; }
      detectNav();
    }, 250);
  ...
  /** @type {Record<string,{name:string,extractedText:string,plagIds?:{strong:string[],weak:string[]}}>} */
