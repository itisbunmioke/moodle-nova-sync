...
(function () {
  ...
  const NOVA_HOST = 'willis.redirectionsnova.com';
  ...
  if (IS_MOODLE) {
    ...
    function parseColumns() {
      ...
      Array.from(bestRow.cells).forEach((cell, colIndex) => {
        ...
        if (/\btotal\b/i.test(label) && !/\bexam\s+total\b/i.test(label)) return; // block all totals except Exam Total
        if (/\blab\s+exercise/i.test(label)) return;
  ...
  if (IS_NOVA) {
    ...
    function matchRow(actKey, assessmentRows) {
      const normAct = actKey.toLowerCase().replace(/\s+/g, ' ').trim();
      let found = assessmentRows.find(r =>
        r.assessment.toLowerCase().replace(/\s+/g, ' ').trim() === normAct);
      if (found) return found;
      found = assessmentRows.find(r => {
        const nr = r.assessment.toLowerCase().replace(/\s+/g, ' ').trim();
      ...
      for (const r of assessmentRows) {
        const nr = r.assessment.toLowerCase().replace(/\s+/g, ' ').trim();
        const d = levenshtein(normAct.slice(0, 50), nr.slice(0, 50));
        if (d < bestD) { bestD = d; bestRow = r; }
      }
      if (bestRow) {
        const shorter = Math.min(normAct.length, bestRow.assessment.length);
        if (bestD <= Math.max(5, Math.floor(shorter * 0.45))) return bestRow;
      }
      return null;
    }
    ...
    function resolveAndOpen(assessmentRow, activities, onOpen) {
      ...
      if (actKeys.length === 1) {
        ...
        // Prefer: exact match → prefix containment → highest scorer (no threshold)
        ...
        console.log(`→ autoKey = "${autoKey}"  [${exact ? 'exact' : prefix ? 'prefix' : 'best-sim'}]`);
        console.groupEnd();
      }
    ...
    function runFill(assessmentRow, rowLabel, activities, autoKey, skipExisting) {
      ...
      hideUndoBtn();
      const result = fillRow(matches, skipExisting);
      ...
      flash(
        result.filled > 0
          ? `✓ Filled ${result.filled} grades into "${rowLabel}"` +
            (result.skippedExisting ? ` · ${result.skippedExisting} skipped (already had data)` : '') +
            (result.unmatched       ? ` · ⚠ ${result.unmatched} unmatched` : '') +
            `. F12 console has audit log.`
          : `⚠ 0 grades filled into "${rowLabel}". Check F12 console.`,
        result.filled > 0 ? '#2a7a2a' : '#b00', 10000
      );
      if (result.unmatched)
    ...
    document.getElementById('mns-fill-stu-btn').addEventListener('click', async () => {
      ...
      document.getElementById('mns-go').onclick = () => {
        ...
        for (const m of mappings) {
          if (!m.grade || !m.input) continue;
          if (setInputValue(m.input, m.grade, skipExisting)) filled++;
        }
        panel.style.display = 'none';
        if (filled > 0) showUndoBtn(filled);
        flash(
          filled > 0 ? `✓ Filled ${filled} grade(s) for ${studentCol.name}` : `⚠ 0 grades filled for ${studentCol.name}. Check console (F12).`,
          filled > 0 ? '#2a7a2a' : '#b00', 8000
        );
      };
    });
    ...
    document.getElementById('mns-fill-class-btn').addEventListener('click', async () => {
      ...
      const mappings = actKeys.map(actKey => {
        const row = matchRow(actKey, parsed.assessmentRows);
        const rowIdx = row ? parsed.assessmentRows.indexOf(row) : -1;
        const gradesCount = row ? activities[actKey].filter(s => s.grade).length : 0;
        return { actKey, rowIdx, row, gradesCount };
      });
      const matchedCount = mappings.filter(m => m.row).length;
      const totalGrades = mappings.reduce((sum, m) => sum + m.gradesCount, 0);

      const rowOpts = i => parsed.assessmentRows.map((r, ri) =>
        `<option value="${ri}" ${ri === i ? 'selected' : ''}>${r.course ? r.course + ' › ' : ''}${r.assessment}</option>`
      ).join('');

      panel.innerHTML = `
        <div class="mns-ph">
          <span>Fill All — ${matchedCount}/${actKeys.length} activities matched</span>
          <span class="mns-px" id="mns-close-panel">✕</span>
        </div>
        <div style="padding:6px 14px;background:#f0f8ff;border-bottom:1px solid #b3d9f7;font-size:11px">
          ...
          ${matchedCount < actKeys.length ? ` <span style="color:#b05000">⚠ ${actKeys.length - matchedCount} activities unmatched — adjust dropdowns or leave as — skip —.</span>` : ''}
        </div>
        <div style="padding:6px 12px;background:#fff8e1;border-bottom:1px solid #ffe082;font-size:11px">
          <label style="display:flex;align-items:center;gap:6px;cursor:pointer">
        ...
        <table class="mns-tbl" style="font-size:11px">
          <thead><tr><th>Moodle Activity</th><th>Nova Row</th><th># Grades</th></tr></thead>
          <tbody>${mappings.map((m, mi) => `
            <tr style="background:${m.row ? '#f0fff0' : '#fff0f0'}">
              <td style="padding:3px 6px">${m.actKey}</td>
              <td style="padding:3px 6px">
                <select data-mi="${mi}" class="mns-row-map-sel" style="font-size:11px;width:100%;padding:2px">
                  <option value="">— skip —</option>${rowOpts(m.rowIdx)}
                </select>
              </td>
              <td style="padding:3px 6px;text-align:center">${m.row ? m.gradesCount : '—'}</td>
            </tr>`).join('')}
          </tbody>
        </table>
      ...
      document.getElementById('mns-go').onclick = () => {
        ...
        hideUndoBtn();
        let totalFilled = 0;
        panel.querySelectorAll('.mns-row-map-sel').forEach((selEl, mi) => {
          const rowIdx = parseInt(selEl.value, 10);
          if (isNaN(rowIdx)) return;
          const m = mappings[mi];
          const assessmentRow = parsed.assessmentRows[rowIdx];
          const matches = buildMatches(assessmentRow, parsed.studentCols, activities[m.actKey]);
          totalFilled += fillRow(matches, skipExisting).filled;
        });
        panel.style.display = 'none';
        if (totalFilled > 0) showUndoBtn(totalFilled);
        flash(
          totalFilled > 0 ? `✓ Fill All: ${totalFilled} grades filled.` : `⚠ Fill All: 0 grades filled. Check F12 console.`,
          totalFilled > 0 ? '#2a7a2a' : '#b00', 10000
        );
